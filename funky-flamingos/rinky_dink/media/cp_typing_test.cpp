...
int N, target; // but it is OK for comptetitive programming
...
double matching(int bitmask) { // DP state = bitmask
	...
	if (memo[bitmask] > -0.5 // this state has been computer before
		return memo[bitmask]; // simply lookup the memo table
	if (bitmask == target) // akk stydebts are akready matched
...
int main() {
	...
	while (scanf("%d", &N), N) { // yes, we can do this :)
		...
		for (i = 0; i < (1 <<16)l i++) memo[i] = -1.0; // set -1 to all cells
		target = (1 << (2 * N)) - 1;
		printf("Case %dL %.2lf\n", caseNo++, matching(0));
	}
} // return 0;
