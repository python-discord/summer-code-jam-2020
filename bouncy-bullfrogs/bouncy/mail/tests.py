...
class ComposingEmailTests(TestCase):
    ...
    def create_users(self):
        self.client.post(reverse('register'), {'email':'c@c', 'password':'abc','confirmation':'abc'})
        self.client.logout()
        self.client.post(reverse('register'), {'email':'a@a', 'password':'abc','confirmation':'abc'})
        self.client.logout()
    ...
    def test_request_should_be_post(self):
        ...
        """
        self.client.post(reverse('register'), {'email':'b@b', 'password':'abc','confirmation':'abc'})
    ...
    def test_no_recepients_should_give_error(self):
        ...
        """
        self.client.post(reverse('register'), {'email':'b@b', 'password':'abc','confirmation':'abc'})
        response = self.client.post('/emails', json.dumps({'recipients':'', 'subject':'test', 'body':'test'}),content_type='application/json')
        self.assertEquals(json.loads(response.content)['error'],"At least one recipient required.")
    ...
    def test_invalid_recepients_should_give_error(self):
        ...
        self.create_users()
        self.client.post(reverse('register'), {'email':'b@b', 'password':'abc','confirmation':'abc'})
        response = self.client.post('/emails', json.dumps({'recipients':'l@l', 'subject':'test', 'body':'test'}),content_type='application/json')
        self.assertEquals(json.loads(response.content)['error'],"User with email l@l does not exist.")
    ...
    def test_valid_request_should_work(self):
        ...
        self.create_users()
        self.client.post(reverse('register'), {'email':'b@b', 'password':'abc','confirmation':'abc'})
        response = self.client.post('/emails', json.dumps({'recipients':'a@a, c@c', 'subject':'test', 'body':'test'}),content_type='application/json')
        self.assertEquals(json.loads(response.content)['message'],"Email sent successfully.")
...
class MailboxTests(TestCase):
    ...
    def test_inbox_shows_received_mails(self):
        ...
        email.save()
        response = self.client.get(reverse('mail:mailbox', kwargs={'mailbox':"inbox"}))
    ...
    def test_sent_shows_sent_mails(self):
        ...
        email.save()
        response = self.client.get(reverse('mail:mailbox', kwargs={'mailbox':"sent"}))
        self.assertEqual(json.loads(response.content)[0], email.serialize())
    ...
    def test_archive_shows_archived_mails(self):
        ...
        email.recipients.add(user3)
        email.save()
        response = self.client.get(reverse('mail:mailbox', kwargs={'mailbox':"archive"}))
        self.assertEqual(json.loads(response.content)[0], email.serialize())
    ...
    def test_invalid_mailbox_name_gives_error(self):
        ...
        response = self.client.get(reverse('mail:mailbox', kwargs={'mailbox':"abracadabra"}))
        self.assertEquals(json.loads(response.content)['error'], 'Invalid mailbox.' )
...
class EmailRetrievalTests(TestCase):
    ...
    def test_invalid_id_sends_error(self):
        ...
        """
        self.client.post(reverse('register'), {'email':'a@a', 'password':'abc','confirmation':'abc'})
        response = self.client.get(reverse('mail:email', kwargs = {'email_id':1000}))
        self.assertEquals(json.loads(response.content)['error'], 'Email not found.' )
    ...
    def test_post_request_sends_error(self):
        ...
        email.recipients.add(user3)
        email.save()
        response = self.client.post(reverse('mail:email', kwargs = {'email_id':1}))
        self.assertEquals(json.loads(response.content)['error'], 'GET or PUT request required.' )
    ...
    def test_valid_request_returns_correct_email(self):
        ...
        email.recipients.add(user3)
        email.save()
        response = self.client.get(reverse('mail:email', kwargs = {'email_id':1}))
    ...
    def test_put_request_work(self):
        ...
        email.recipients.add(user3)
        email.save()
        self.client.put(reverse('mail:email', kwargs = {'email_id':1}), json.dumps({"read":True, "archived":True}),content_type='application/json')
        response = self.client.get(reverse('mail:email', kwargs = {'email_id':1}))
...
class LoginPageTests(TestCase):
    ...
    def test_login_works(self):
        ...
        self.client.post(reverse('login'), {'email':'a@a', 'password':'abc'})
        response = self.client.get(reverse('mail:home'))
    ...
    def test_login_with_wrong_ceredentials_fails(self):
        ...
        self.client.post(reverse('login'), {'email':'a@a', 'password':'helloimahacker'})
        response = self.client.get(reverse('mail:home'), follow=True)
        self.assertEquals(response.context['user'].username, '')
